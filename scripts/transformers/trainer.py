...
def top_vocab(version, tokenizer, dataset_input_ids, vocab_k=50):
    
    ## training 
    if version=="train": 
        vocab_64 = [9, 33, 38, 34, 32, 7, 47334, 73, 91, 89, 75, 80, 65, 100, 98, 136, 104, 116, 157, 145, 61, 58, 87, 176, 184, 189, 111, 213, 192, 218, 232, 182, 238, 245, 179, 248, 246, 253, 264, 250, 275, 47329, 262, 276, 283, 249, 193, 263, 299, 277]
        return vocab_64
    ## validation 
    ...
    ## get every punctuation tokens from tokenizer vocab 
    punc = string.punctuation
    vocabs = tokenizer.get_vocab()
    punc_tokens = []
    for word in vocabs:
        chars = word.split('</w>')[0]
        for ch in chars:
            if ch in punc: 
                punc_tokens.append(vocabs.get(word)) 
                break

    ## get every tokens that eliminated punctuations 
    tokens = []
    for line in dataset_input_ids:
        for token_id in line:
            if token_id not in punc_tokens:
                tokens.append(token_id)
    counter = Counter(tokens)
    most_occur = counter.most_common(vocab_k)

    most_occur_words=[] 
    for word in most_occur:
        most_occur_words.append(word[0])

    return most_occur_words

# randomly delete the token
def apply_deletion(tgt):
    ### using torch 
    rnd = torch.rand(len(tgt)) 
    rndthreshold = 0.1
    remove = rnd > rndthreshold  # save only token of larger than threshold. 
    
    # take only True (rnd > rndthreshold) index tokens. (= Drop token of prob < 0.1) 
    src = torch.take(tgt, torch.squeeze(remove.nonzero()).cuda())
...
# randomly shuffle the order of words locally 
def apply_permutation(tgt, permutation_distance=3):
    ### using torch 
    length = len(tgt)

    offset = torch.randint(0,permutation_distance + 1, (length,)).cuda()
    indices = torch.arange(length).cuda() + offset
    sentence = torch.take(tgt, torch.argsort(indices)).cuda()

    return sentence

# add new token which is saved in corpus 
def apply_addition(tgt, vocab):
    
    ### using pytorch
    rnd = torch.rand(len(tgt))
    rndthreshold = 0.1 
    insert = rnd > rndthreshold

    # if token random number is smaller than threshold, insert new word before the token 
    insert_idx = [i for i in range(len(insert)) if insert[i]!=True]
    insert_words = [vocab[int(torch.randint(len(vocab),(1,)))] for i in insert_idx]
    
    src = np.insert(tgt.cpu(), insert_idx, insert_words).cuda()
...
TODO: 
    max_length is fixed 
'''
def apply_noise(version, tokenizer, datasets, max_length=512):
    cleans = datasets['input_ids'] 
    ...
    ## training 
    if version=="train": 
        freq_vocab = top_vocab("train", tokenizer, cleans, 50)
    ## validation 
    if version=="validation": 
        freq_vocab = top_vocab("validation", tokenizer, cleans, 50)
    ## test 
    if version=="test":
        freq_vocab = top_vocab("test", tokenizer, cleans, 50)

    for idx in range(len(cleans)):
        input_id = cleans[idx][:cleans[idx].tolist().index(tokenizer.eos_token_id) ]  
        label = cleans[idx][:cleans[idx].tolist().index(tokenizer.eos_token_id) ] 

        rnd = np.random.random()
        clean_threshold = 0.5

        if rnd > clean_threshold: 
            # 1. apply permutation
            input_id = apply_permutation(input_id)
            # 2. apply deletion
            input_id = apply_deletion(input_id)
            # 3. apply addition 
            input_id = apply_addition(input_id, freq_vocab)

        input_id = torch.cat((input_id, torch.tensor([0], device='cuda') ))
        label = torch.cat((label, torch.tensor([0], device='cuda') ))

        input_id = F.pad(input_id, (0, max_length-len(input_id)), mode='constant', value=tokenizer.pad_token_id)
        label = F.pad(label, (0, max_length-len(label)), mode='constant', value=-100)

        input_ids.append(input_id)
        labels.append(label)
    
    return input_ids, labels  # `t_input_ids`: src (clean), `t_labels`: tgt (noise)
...
class LabeledDataset(data.Dataset):
    def __init__(self, version, tokenizer, dataset=None):
        self.max_length=1024
        self.data = {}

        self.attention_mask = dataset['attention_mask']
        self.input_ids, self.labels = apply_noise(version, tokenizer, dataset, self.max_length)

        self.data['input_ids'] = self.input_ids
        self.data['attention_mask'] = self.attention_mask
        self.data['labels'] = self.labels 

        input_ids = torch.stack(self.data['input_ids'])
        self.data['input_ids'] = input_ids
        labels = torch.stack(self.data['labels'])
        self.data['labels'] = labels

        self.decoder_input_ids = shift_tokens_right( labels, tokenizer.pad_token_id, tokenizer.pad_token_id )
        self.data['decoder_input_ids'] = self.decoder_input_ids
...
class Trainer:
    ...
    def get_test_dataloader(self, test_dataset: Dataset) -> DataLoader:
        ...
        ########## ADD 
        test_input_ids = torch.tensor(test_dataset['input_ids']).cuda() # .to(device="cuda:0")
        test_attention_mask = torch.tensor(test_dataset['attention_mask']).cuda() # .to(device="cuda:0")

        test_data = {}
        test_data['input_ids'] = test_input_ids
        test_data['attention_mask'] = test_attention_mask
        
        test_labeled_dataset = LabeledDataset("test", self.tokenizer, test_data)
        test_dataset = Dataset.from_dict(test_labeled_dataset.data)
        ############
    ...
    def _inner_training_loop(
        ...
        ############ ADD (create `labels` to valid befor goto epoch iter)
        # validation
        eval_input_ids = torch.tensor(self.eval_dataset['input_ids']).cuda()
        eval_attention_mask = torch.tensor(self.eval_dataset['attention_mask']).cuda()

        eval_data = {}
        eval_data['input_ids'] = eval_input_ids
        eval_data['attention_mask'] = eval_attention_mask
        
        valid_labeled_dataset = LabeledDataset("validation", self.tokenizer, eval_data)
        self.eval_dataset = Dataset.from_dict(valid_labeled_dataset.data)
        ##############
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                ########## ADD  (add noise here in mini-batch!)
                train_labeled_dataset = LabeledDataset("train", self.tokenizer, inputs)
                inputs = train_labeled_dataset.data
                ##########
